...
func runTests(useGopath bool, t *testing.T, when spec.G, it spec.S) {
	...
	it.Before(func() {
		...
		initModuleFunc = func() {
			...
			err := ioutil.WriteFile(filepath.Join(baseDir, "go.mod"), []byte("module github.com/chintanparikh/counterfeiter/fixtures"), 0755)
			Expect(err).ToNot(HaveOccurred())
		}
		...
		t := func(interfaceName string, filename string, subDir string, files ...string) {
			when("working with "+filename, func() {
				...
				it("succeeds", func() {
					...
					if !useGopath {
						WriteOutput([]byte(fmt.Sprintf("module github.com/chintanparikh/counterfeiter/fixtures%s\n", suffix)), filepath.Join(baseDir, "go.mod"))
					}
					f, err := generator.NewFake(generator.InterfaceOrFunction, interfaceName, fmt.Sprintf("github.com/chintanparikh/counterfeiter/fixtures%s", suffix), "Fake"+interfaceName, "fixturesfakes", baseDir)
					Expect(err).NotTo(HaveOccurred())
					b, err := f.Generate(true) // Flip to false to see output if goimports fails
					Expect(err).NotTo(HaveOccurred())
					if writeToTestData {
		...
		when("working with duplicate packages", func() {
			t := func(interfaceName string, offset string, fakePackageName string) {
				when("working with "+interfaceName, func() {
					...
					it("succeeds", func() {
						pkgPath := "github.com/chintanparikh/counterfeiter/fixtures/dup_packages"
						if offset != "" {
